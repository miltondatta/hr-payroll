...
class Appraisal extends CI_Controller{
    ...
    public function appraisalEmployee(){
        if($this->session->userdata('user_login_access') != false){
            ...
            $data['appraisal_employee_data'] = array();
    ...
    public function getAppraisalCategory(){
        if($this->session->userdata('user_login_access') != false){
            ...
            $appraisal_exists = $this->Appraisal_model->getAppraisalByEmployeeAndFinancialYear($em_id, $financial_year);
        ...
        if($this->session->userdata('user_login_access') != false){
            ...
            foreach($category_id as $key => $value){
                if($this->input->post('category_rating_' . $key)){
                    ...
                    $index                  = array_search($this->input->post('category_rating_' . $key),
                                                           $this->input->post('category_rating_array_' . $key));
    ...
    public function FilteredEmployeeAppraisal(){
        $employee_id = $this->input->post('employee_id');
        $fin_year    = $this->input->post('fin_year');
        
        $data['appraisal_employee_data'] = $this->Appraisal_model->filterAppraisalEmployeeData($employee_id,$fin_year);
        $this->load->view('backend/appraisal-employee-partial', $data);
    }
