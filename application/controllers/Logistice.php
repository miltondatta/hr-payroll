...
class Logistice extends CI_Controller
{
    ...
    public function Add_Logistic_Support()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                redirect('Logistice/logistic_support');
            } else {
                $data = array(
                    'asset_id' => $logid,
                    'assign_id' => $assignid,
        ...
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (empty($id)) {
                    ...
                    $this->session->set_flashdata('feedback', 'Successfully Added');
                    redirect(base_url('Logistice/Assets_Category'));
                } else {
    ...
    {
        if ($this->session->userdata('user_login_access') != False) {
            $this->logistic_model->assets_category_delete($id);
            $this->session->set_flashdata('feedback', 'Successfully Deleted');
            redirect('Logistice/Assets_Category');
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    {
        if ($this->session->userdata('user_login_access') != False) {
            $this->logistic_model->assets_delete($id);
            $this->session->set_flashdata('feedback', 'Successfully Deleted');
            redirect('Logistice/All_Assets');
        } else {
            redirect(base_url(), 'refresh');
