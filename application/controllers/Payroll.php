...
class Payroll extends CI_Controller{
    ...
    public function Add_Sallary_Type(){
        if($this->session->userdata('user_login_access') != false){
            ...
            $this->form_validation->set_rules('typename', 'Type name',
    ...
    public function Add_Sallary_Pay(){
        if($this->session->userdata('user_login_access') != false){
            ...
            $this->form_validation->set_rules('basic', 'Employee Basic',
                                              'trim|required|min_length[2]|max_length[7]|xss_clean');
    ...
    public function load_employee_Invoice_by_EmId_for_pay(){
        if($this->session->userdata('user_login_access') != false){
            ...
            $obj_merged = (object)array_merge((array)$employee_info, (array)$salaryvaluebyid, (array)$salarypaybyid,
                                              (array)$salaryvalue, (array)$loanvaluebyid);
                        ...
                        <div class='card card-body'>
                            <div class='row'>
                                <div class='col-md-4 col-xs-6 col-sm-6'>
                                    <img src='$base/assets/images/$settingsvalue->sitelogo' style=' width:180px; margin-right: 10px;' />
                                </div>
    ...
    public function GetSalaryByWorkdays(){
        ...
        if($this->session->userdata('user_login_access') != false){
            ...
            // Employee actually worked
            $employee_actually_worked =
                $this->hours_worked_by_employee($employeeID, $start_date, $end_date);  // in hours
            ...
            if($this->form_validation->run() == false){
                ...
                if(isset($payID) && $payID > 0){
                    ...
                    if($payment_status == "Paid"){
                        $this->session->set_flashdata('error', 'Salary already been paid');
    ...
    public function load_employee_by_deptID_for_pay(){
        ...
        if($this->session->userdata('user_login_access') != false){
            ...
            if(count($employees) > 0){
                ...
                $data                             = array();
                $data['basic_salary']             = $employee_salary ?? 0;
                $data['total_work_hours']         = $total_work_hours ?? 0;
                $data['employee_actually_worked'] = $employee_actually_worked[0]->Hours ?? 0;
                $data['addition']                 = $addition ?? 0;
                $data['diduction']                = $diduction ?? 0;
